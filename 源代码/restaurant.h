...
public:
    ...
    QHash <QString,QString> AdminiHash;//从数据库读取管理员账户
    ...
    bool FindCustomerPwd(const QString &pwd);//查找顾客密码
    bool FindWaiterName(const QString &name);//查询服务员名
    bool FindWaiterPwd(const QString &pwd);//查询服务员密码
    bool FindCookName(const QString &name);//查询厨师名
    bool FindCookPwd(const QString &pwd);//查询厨师密码
    bool FindManagerName(const QString &name);//查询餐厅经理用户名
    bool FindManagerPwd(const QString &pwd);//查询餐厅经理密码
    bool FindAdminName(const QString &name);//查询系统管理员用户名
    bool FindAdminPwd(const QString &pwd); //查询系统管理员密码
    ...
    void GetDeskDish(const QString &dishname, const QString &dishprice, const int &desknum, const int dishnum);//一条条录入菜单(餐桌的菜),可以看看能不能删
