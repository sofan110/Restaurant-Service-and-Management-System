...
void Restaurant::InCookHash()
{
 ...
 void Restaurant::InWaiterHash()
 {
     ...
     while(query.next())
     {
        ...
        w.getPwd(query.value(1).toString());
 ...
 void Restaurant::InAdminiHash()
 {
     ...
     while(query.next())
     {
        AdminiHash.insert(query.value(0).toString(),query.value(1).toString());
...
void Restaurant::InManagerHash()
{
 ...
 void Restaurant::InsertCustomer()
 {
     ...
     for(i=CustomerHash.begin();i!=CustomerHash.end();++i)
      {
            DataBase::Insitial()->inserct(i.value().returnCustomName(),i.value().returnPwd());
      }
 }
 ...
 void Restaurant::InsertCook()
 {
     ...
     for(i=CookHash.begin();i!=CookHash.end();++i)
      {
           DataBase::Insitial()->insertInDishMap(i.value().ReturnCookName(),i.value().ReturnCookPwd());
      }
...
bool Restaurant::FindCustomerName(const QString &name)
{
    ...
    for(i=CustomerHash.begin();i!=CustomerHash.end();++i)
    {
        if(i.value().returnCustomName() == name)
             return true;
    }
...
bool Restaurant::FindCustomerPwd(const QString &pwd)
{
    QHash<int,Customer>::iterator i;

    for(i=CustomerHash.begin();i!=CustomerHash.end();++i)
    {
        if(i.value().returnPwd() == pwd)
             return true;
    }
...
bool Restaurant::FindWaiterPwd(const QString &pwd)
{
    QHash<int,Waiter>::iterator i;

    for(i=Restaurant::Instance()->WaiterHash.begin();i!=Restaurant::Instance()->WaiterHash.end();++i)
    {
        if(i.value().ReturnWaiterPwd() == pwd)
             return true;
    }
...
bool Restaurant::FindCookPwd(const QString &pwd)
{
    QHash<int,Cook>::iterator i;

    for(i=Restaurant::Instance()->CookHash.begin();i!=Restaurant::Instance()->CookHash.end();++i)
    {
        if(i.value().ReturnCookPwd() == pwd)
             return true;
    }
...
bool Restaurant::FindManagerPwd(const QString &pwd)
{
    QHash<QString,QString>::iterator i;
...
bool Restaurant::FindAdminName(const QString &name)//查询系统管理员用户名
{
    QHash<QString,QString>::iterator i;

    for(i=Restaurant::Instance()->AdminiHash.begin();i!=Restaurant::Instance()->AdminiHash.end();++i)
    {
        if(i.key() == name)
             return true;
    }
...
bool Restaurant::FindAdminPwd(const QString &pwd) //查询系统管理员密码
{
    QHash<QString,QString>::iterator i;
